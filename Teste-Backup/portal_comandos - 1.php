...
function mostrarnews($pagina = 0)	{
	if(isset($_SESSION['id_usuario']) && is_numeric($_SESSION['id_usuario'])){
		if(($_SESSION["nivel"] >= "1") && ($_SESSION["nivel"] <= "49")){
			...
			echo "</DIV><div class='small-12 columns' style='margin-top:35px;'><center><FONT class=links>\n";
			if($pagina > 0){
				$url = "https://www.canalicara.com/portal/portal_admin.php?action=mostrarnews&pagina=".($pagina - 1);	
				echo "<a href='$url'><FONT color=#808080>Anterior</a></FONT> ";
			}
			$atual = ($pagina + 1);

			for($i = 0; $i < 12 $i++){
				$pg = $i+1;
				if(($pg + 6 - $i) > 0 && ($pg + 6 - $i) < $paginas){
					$url = "https://www.canalicara.com/portal/portal_admin.php?action=mostrarnews&pagina=$i";
					echo $pg == $atual ? "<a href='$url'><Font color=#808080>$pg</Font></a> | " : "<a href='$url'><Font color=#808080><b>[".$pg."]</b></Font></a> | " ;
				}
			}
			...
			if($pagina < $paginas-1){
				$url = "https://www.canalicara.com/portal/portal_admin.php?action=mostrarnews&pagina=".($pagina + 1);
				echo " <a href='$url'><FONT color=#808080>Próxima</FONT></a>";
			}	
			echo "</FONT></center></div><br><br>\n";
		}
